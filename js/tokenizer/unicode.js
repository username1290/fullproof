 ...
 */
net.kornr.unicode.lowercase =
    new net.kornr.unicode.Normalizer(norm_lowercase_data);


/**
 * @type {net.kornr.unicode.Normalizer}
 */
net.kornr.unicode.lowercase_nomark =
    new net.kornr.unicode.Normalizer(norm_lowercase_nomark_data);
 ...
 */
net.kornr.unicode.tokenize = function(text, callback) {
  var start = 0;
  var len = 0;
  var max = text.length;
  for (var i = 0; i < max; ++i) {
    ...
    if (!net.kornr.unicode.is_letter_number(c)) {
      len = i - start;
      if (len) {
        callback(start, len);
      }

...
goog.provide('fullproof.BooleanEngine');
...
goog.inherits(fullproof.BooleanEngine, fullproof.AbstractEngine);
 ...
 * @protected
 */
fullproof.BooleanEngine.prototype.lookup = function(text, callback, arrayOfIndexUnits, mode) {
  ...
  unit.analyzer.parse(text, fullproof.make_synchro_point(function(array_of_words) {
    ...
    var lookup_synchro = fullproof.make_synchro_point(function(rset_array) {
      ...
      if (curset.getSize() == 0) {
        if (arrayOfIndexUnits.length > 0) {
          this.lookup(text, callback, arrayOfIndexUnits, mode);
        } else {
          callback(false);
        }
      } else {
 ...
 */
fullproof.BooleanEngine.prototype.lookup = function(text, callback) {
  this.lastResultIndex = 0;
  this.lookup(text, callback, this.getIndexUnits(), this.booleanMode);
};
